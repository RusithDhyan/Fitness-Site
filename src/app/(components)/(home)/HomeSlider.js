...
export default function HomeSlider({ sectionRef }) {
  ...
  useEffect(() => {
    ...
    typingInterval.current = setInterval(() => {
      if (charIndex < homeSlider[currentIndex].title.length) {
        setTypedText((prev) => prev + homeSlider[currentIndex].title[charIndex]);
        charIndex++;
      } else {
        clearInterval(typingInterval.current);
      }
    ...
    <div className="w-full">
      <Swiper
        ...
        onSlideChange={(swiper) => {
          setCurrentIndex(swiper.realIndex); // Update slide index
        }}
        breakpoints={{
          640: { slidesPerView: 1 },
          768: { slidesPerView: 1 },
